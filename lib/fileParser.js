...
export default class FileParser {
    ...
    extractCode(AST) {
        ...
        matches = esquery.match(AST, esquery.parse(`ObjectExpression > Property[key.name=mixins]`));
        if (matches && matches[0] && matches[0].value) {
            let node = matches[0].value;
            if (node.type === "Literal") {
                // me.addRequire(node.value);
                requires.mixins = [node.value]
            } else if (node.type === "ArrayExpression") {
                requires.mixins = []
                node.elements.forEach((element) => {
                    // me.addRequire(element.value);
                    requires.mixins.push(element.value)
                });
            }
        }
     ...
     */
    extractComments(AST) {
        if(!AST.comments) console.warn('no comments...')
